...
async function runTests() {
    server.listen(0, async () => {
        ...
        if (receivedWebhooks.length === 0) {
            console.log('[INFO] Server response received cleanly. (Note: If backend is live on Render without BOT_SERVICE_URL pointing to local mock, webhooks dispatch to configured BOT_SERVICE_URL).');
        } else {
            let passed = true;
        ...
        console.log('\n[SUCCESS] Test script run complete!');
        server.close();
        process.exit(0);
    });
}
